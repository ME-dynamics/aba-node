...
describe("operators", () => {
  it("should return equality string", () => {
    ...
    const staticEqualStr = equal({ argument: "id", staticValue: 1 });
    expect(dynamicEqualStr).toBe("id = :id");
    expect(staticEqualStr).toBe("id = 1");
  });
  it("should return IN string", () => {
    const dynamicInStr = IN({ argument: "id", dynamicValue: true });
    const staticInStr = IN({ argument: "id", tuple: [1, 2, 3] });
  ...
  it("should return greaterThan string", () => {
    const greaterThanStr = greaterThan({
      argument: "age",
      equalAndGreater: true,
      dynamicValue: true,
    });
    const greaterThanStaticStr = greaterThan({
      argument: "age",
      equalAndGreater: false,
      staticValue: 20,
    });
